...
export type AsyncWorkerClient<T extends IProcMap> = {
    [K in keyof T]: T[K] extends PromiseFunc | Func ? (...args: Parameters<T[K]>) => Promise<ReturnType<T[K]>> : T[K] extends Task<any, any, infer E> ? () => E extends Promise<any> ? E : Promise<E> : never;
} & {
    exit: () => Promise<void>;
};
export {};
